...
angular.module('demoApp')
  .controller('MainCtrl', function ($scope) {
  	...
  	$scope.texts = [{"title":"THE PIONEERS OF MICRONUTRIENT TECHNOLOGY IN INDIA","link":"#/profile"},{"title":"OUR VISION: PRODUCTS THAT WORK.","link":"#/products"},{"title":"OUR WORK: EDUCATING THE FARMERS.","link":"#/recommendation"}];
    $scope.headCaption = $scope.texts[0].title;
    $scope.pageLink = $scope.texts[0].link
	...
	BoxesFx.prototype._navigate = function( dir ) {
		...
		if( dir === 'next' ) {
			this.current = this.current < this.panelCount - 1 ? this.current + 1 : 0;	
			// console.log(this.current + $scope.texts[this.current]);
			$scope.headCaption = $scope.texts[this.current].title;
			$scope.pageLink = $scope.texts[this.current].link;
			console.log($scope.headCaption);
			// $scope.apply();
			$scope.$evalAsync(
                        function( $scope ) {
 							console.log($scope.headCaption);
			...
			this.current = this.current > 0 ? this.current - 1 : this.panelCount - 1;
			$scope.headCaption = $scope.texts[this.current].title;
			$scope.pageLink = $scope.texts[this.current].link;
			console.log($scope.headCaption);
			// $scope.apply();
